...
sequenceDiagram
    ...
    O->>FE: Input Data Hewan (Nama, Jenis,Ras, dll)
    FE->>BE: Upload Data
    BE-->>FE: Return dataHash
...
```mermaid
sequenceDiagram
    participant PC as Pet Clinic
    participant FE as Frontend
    participant BE as Backend
    participant SC as Smart Contract

    PC->>FE: Cari Pet & Input Catatan Vaksinasi
...
sequenceDiagram
    ...
    FE-->>P: Tampilkan Profil Hewan (Blockchain Verified)
```
