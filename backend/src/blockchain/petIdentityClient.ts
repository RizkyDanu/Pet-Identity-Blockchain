...
export const confirmAddMedicalRecordTx = async (params: {
  ...
  const recordId = toNumber(args?.recordId ?? args?.[0], "recordId");
  const petId = toNumber(args?.petId ?? args?.[1], "petId");
  const dataHash = decodeBytes32Arg(args?.dataHash ?? args?.[2]);
  
  // Perbaikan: Ambil alamat pengirim (clinic) dari event/metadata
  // Karena verifiedBy mungkin address(0) jika logika PENDING dipertahankan
  const actor = decodeAddressArg(args?.verifiedBy ?? args?.[4] ?? metadata.from);
  
  const expectedWallet = normalizeWalletAddress(params.expectedWalletAddress);

  if (Number(petId) !== params.expectedPetId) {
