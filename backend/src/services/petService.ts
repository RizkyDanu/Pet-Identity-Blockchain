...
export const initiateTransfer = async (
  ...
  if (!updatedPet) {
    throw new AppError("Data hewan tidak ditemukan", 404);
  }
...
export const acceptTransfer = async (petId: number, newOwnerId: number) => {
  ...
  const petRepo = AppDataSource.getRepository(Pet);
  const pet = await petRepo.findOne({ where: { id: petId } });
  if (!pet) throw new AppError("Data hewan tidak ditemukan", 404);
  ...
  await createNotification({
    ...
    message: `Kepemilikan hewan ${pet.name} kini sudah diterima pemilik hewan yang baru.`,
  });
  await createNotification({
    userId: transfer.toOwnerId,
    title: "Transfer diterima",
    message: `Anda kini tercatat sebagai pemilik hewan ${pet.name}.`,
  });
...
export const createCorrectionRequest = async (params: {
  ...
  if (!pet || pet.ownerId !== params.ownerId) {
    throw new AppError("Data hewan tidak ditemukan atau akses ditolak", 404);
  }

  if (!(params.fieldName in correctionFieldMap)) {
    throw new AppError("Bagian tidak dapat dikoreksi", 400);
  }
